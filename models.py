...
class ResearchPath(BaseModel):
    ...
    response_prompt: ClassVar[PromptModel] = PromptModel(
        ...
        task="Your task is to take the facts provided in <facts> XML tags and user preferences in <preferences> XML tags above and return a response designed to provide information. Reference SPECIFIC user preferences if you know them for but do not assume unless it can be explicitly found in provided context. Then provide the facts as if they were travel suggestions.",
        flavor="Skip the preamble and do not mention your instructions. Do not keep repeating the context, just provide information, and strive for simplicity and clarity. Use natural sounding and conversational language. Avoid slang, jargon, or other colloquialisms that make you sound like an aging baby boomer. Be concise, keep your messages short, and get to the point. Three sentences at most."
    )
...
class ConversationState(BaseModel):
    state: str = Field(description="Identifies the state of the conversation. Choose one value from this list: ['wait', 'research', 'question']. Choose wait if last message is a user talking to another user, question if there's a pointed question you need to answer, research if there is indecision and/or you need to move conversation toward recommendation.")
